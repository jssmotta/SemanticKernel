...
public class MySqlQueryPlugin
{
    private readonly DatabaseSchema _databaseSchema;
    public MySqlQueryPlugin(DatabaseSchema databaseSchema)
    {
        _databaseSchema = databaseSchema;
    }

    [KernelFunction]
    [Description("Generates a MySQL query based on the user's natural language request and database schema")]
    public async Task<string> GenerateSqlQuery(
     [Description("User's natural language request for a MySQL query")] string input,
     Kernel kernel,
...
You are a Data Analyst and MySQL expert who helps translate natural language questions about databases into MySQL queries.
...
- Format your output as a clear explanation followed by the SQL query in 
•	For queries that involve aggregations, add appropriate GROUP BY clauses
•	Handle NULLs appropriately
•	Use backticks (`) around table and column names when necessary
•	MySQL is case-sensitive for table names on some platforms, so maintain the exact case from the schema");
    ...
    {
        var sb = new StringBuilder(); sb.AppendLine($"Database: {_databaseSchema.DatabaseName}"); sb.AppendLine();
